...
export default async (req) => {
  ...
  if (phase === "list") {
    ...
    const need = [];
    for (const a of all.filter(isRide).slice(0, 15))
      if (!(await readJSON(`streams/${a.id}.json`))) need.push(a.id);
  ...
  if (phase === "streams") {
    ...
    if (await readJSON(`streams/${id}.json`)) return json({ ok: true, cached: true });
    const tok = await stravaToken();
    if (!tok) return json({ error: "not_connected" }, 401);
    ...
    if (!r.ok) return json({ ok: false, skip: true, status: r.status });
    const raw = await r.json(); const N = 1100;
    await writeJSON(`streams/${id}.json`, {
      time: ds(raw.time?.data, N), watts: ds(raw.watts?.data, N), hr: ds(raw.heartrate?.data, N),
      cad: ds(raw.cadence?.data, N), vel: ds(raw.velocity_smooth?.data, N),
      alt: ds(raw.altitude?.data, N), latlng: ds(raw.latlng?.data, N) });
    return json({ ok: true });
  }

  // phase === "compute": everything from stored data only — fast, no Strava calls
  ...
  // dynamic season window: seasonStart → 4 weeks past today, or the last planned event
  const s0 = new Date((prof.seasonStart || "2026-03-16") + "T00:00:00Z");
  const lastEvent = (prof.events || []).reduce((mx, e) => Math.max(mx, +new Date(e.date)), 0);
  const end = Math.max(Date.now() + 28 * 864e5, lastEvent);
  const NW = Math.max(8, Math.min(40, Math.ceil((end - s0) / (7 * 864e5))));
  const weeks = new Array(NW).fill(0), wkCount = new Array(NW).fill(0);
  for (const [d, v] of Object.entries(daily)) {
    const wi = Math.floor((new Date(d + "T12:00:00Z") - s0) / (7 * 864e5));
    if (wi >= 0 && wi < NW) weeks[wi] += v; }
  ...
  for (const a of seasonActs) { const wi = Math.floor((new Date(a.start_date) - s0) / (7 * 864e5));
    if (wi >= 0 && wi < NW) wkCount[wi]++; }
  const curWeek = Math.max(0, Math.min(NW - 1, Math.floor((Date.now() - s0) / (7 * 864e5))));
  ...
  const weekRides = Array.from({ length: NW }, () => []);
  for (const a of seasonActs) { const wi = Math.floor((new Date(a.start_date) - s0) / (7 * 864e5));
    if (wi >= 0 && wi < NW) { const det = rides.find(r => r.id === a.id);
