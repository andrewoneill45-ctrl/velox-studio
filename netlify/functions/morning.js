...
export default async () => {
  ...
  try {
    await fetch(`${base}/api/sync?phase=list`).catch(() => {});
    await fetch(`${base}/api/sync?phase=compute`);
    const cards = await readJSON("cards.json", {});
    const today = new Date().toISOString().slice(0, 10);
    for (const mode of ["readiness", "weekly"]) {
      try { const r = await fetch(`${base}/api/coach`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ mode }) });
